...
public class APanel extends JPanel implements MouseListener, MouseMotionListener, KeyListener {
	...
	public void paintComponent(Graphics g){
		g.setColor(new Color(209, 226, 255));
		...
		g.setColor(new Color(192, 255, 255, 128));
		if(bgRects[0] != -1){
		...
		if(mode == "title"){
			...
			drawCenteredString(g, 200, 100, "Arithmetr");

			g.setFont(new Font("SansSerif", Font.PLAIN, 15));
			drawCenteredString(g, 200, 150, "A GAME OF MATHEMATICAL EFFICIENCY");
			
			g.drawRect(100, 500, 200, 50);
			
			if(mouseX >= 100 && mouseX <= 300 && mouseY >= 500 && mouseY <= 550) g.setColor(new Color(192, 255, 255));
			else g.setColor(new Color(128, 255, 255));
			...
			if(!hasQuestion){
				if(Math.random()>0.5 + 2.5/(level+1)/5) subtraction = true;
				else subtraction = false;
				
				position = (int) Math.round(Math.random()*2);
				
				num1 = (int) Math.round(Math.random()*(level+1));
				num2 = (int) Math.round(Math.random()*(level+1));
				
				if(position == 2){
					if(subtraction && num2 > num1){
						int plc = num2;
						num2 = num1;
						num1 = plc;
					}
					
					if(subtraction) ans = num1-num2;
					else ans = num1+num2;
				} else if(position == 1){
					if(subtraction && num2 > num1){
						int plc = num2;
			...
			if(waiting){
				...
				if(subtraction)drawCenteredString(g, 140, 325, "-");
				else drawCenteredString(g, 140, 325, "+");
				...
				if(position == 0){
					...
					g.setColor(new Color(0, 0, 0));
					drawCenteredString(g, 200, 325, Integer.toString(num1));
					drawCenteredString(g, 320, 325, Integer.toString(num2));
					g.setFont(inSize(g));
	...
	public void keyPressed(KeyEvent e) {
		if(mode == "game" && !waiting){
			if(e.getKeyCode() >= 48 && e.getKeyCode() <= 57){
				...
				if(in.equals(Integer.toString(ans))){
					hasQuestion = false;
					level++;
